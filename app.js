...
async function loadAllData() {
  const [confi, sellers, clients, products, publi, support] = await Promise.all([
    fetchSheet("confi"),
    fetchSheet("usuarios"),
    fetchSheet("clientes"),
  ...
  state.publi = publi
    ...
    .map(r => ({
      ...
      linea_1: String(r.linea_1 || "").trim(),
      linea_2: String(r.linea_2 || "").trim(),
      linea_3: String(r.linea_3 || "").trim(),
      linea_4: String(r.linea_4 || "").trim()
    }));
...
function renderCarousel() {
  ...
  wrap.innerHTML = `
    <div class="c-shell">
      <div class="c-track-outer">
        <div class="c-track" id="cTrack">
          ${slides.map(s => `
            <div class="c-slide" style="background-image:url('${esc(s.imagen)}')">
              <div class="c-content">
                ${s.linea_1 ? `<div class="c-line1">${esc(s.linea_1)}</div>` : ""}
                ${s.linea_2 ? `<div class="c-line2">${esc(s.linea_2)}</div>` : ""}
                ${s.linea_3 ? `<div class="c-line3">${esc(s.linea_3)}</div>` : ""}
                ${s.linea_4 ? `<div class="c-line4">${esc(s.linea_4)}</div>` : ""}
              </div>
            </div>`).join("")}
        </div>
        ${multi ? `
          <button class="c-arrow cleft"  id="cPrev" type="button" aria-label="Anterior">&#8249;</button>
          <button class="c-arrow cright" id="cNext" type="button" aria-label="Siguiente">&#8250;</button>` : ""}
      </div>
      ${multi ? `<div class="c-dots" id="cDots"></div>` : ""}
    </div>`;

  if (multi) initCarousel(slides.length);
}

function initCarousel(total) {
  function updateC() {
    const track = $("#cTrack");
    if (track) track.style.transform = `translateX(-${_cIdx * 100}%)`;
    document.querySelectorAll(".c-dot").forEach((d, i) => d.classList.toggle("active", i === _cIdx));
  }
  function goC(n) {
    _cIdx = (n + total) % total;
    updateC();
    clearInterval(_cTimer);
    _cTimer = setInterval(() => goC(_cIdx + 1), 4200);
  }

  const dotsEl = $("#cDots");
  if (dotsEl) {
    dotsEl.innerHTML = Array.from({ length: total }, (_, i) =>
      `<button class="c-dot${i === 0 ? " active" : ""}" type="button" data-ci="${i}" aria-label="Slide ${i + 1}"></button>`
    ).join("");
    dotsEl.addEventListener("click", e => {
      const d = e.target.closest("[data-ci]");
      if (d) goC(Number(d.dataset.ci));
    });
  }

  const prev = $("#cPrev");
  const next = $("#cNext");
  if (prev) prev.addEventListener("click", () => goC(_cIdx - 1));
  if (next) next.addEventListener("click", () => goC(_cIdx + 1));
