...
function renderFinanceClientsD9() {
  ...
  list.innerHTML=clients.map(client=>`<button type="button" data-finance-client="${esc(client.id)}" class="option-item"><strong>${esc(client.nombre)}</strong><small>${esc([client.direccion,client.ciudad].filter(Boolean).join(" · "))}</small></button>`).join("")||'<p class="mini-text">Sin clientes para esta búsqueda.</p>';
